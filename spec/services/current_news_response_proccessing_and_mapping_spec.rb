      ...
      context 'single_news is NOT in CurrentNews table' do
        ...
        context 'should go through statements in unless condition' do
          ...
          it "single_news['category'] array is not empty" do
            ...
            expect(category_name).to eql('regional')
            category_id = CurrentNewsResponseProccessingAndMapping.get_category_id(single_news)
            # expect(news_attributes).to eq({
            ...
            expect(call()).to eql(news_attributes)  
          end
        end
      end
